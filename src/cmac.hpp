...
class cmac {
public:
  cmac(xrt::ip &cmac) : cmac_(cmac) {}

  cmac(xrt::ip &&cmac) : cmac_(std::move(cmac)) {}

  void set_reset() { cmac_.write_register(reset_reg, 1); }

  void clear_reset() { cmac_.write_register(reset_reg, 0); }

  void enable_rx() {
    auto val = cmac_.read_register(conf_rx);
    val |= 1;
    cmac_.write_register(conf_rx, val);
  }
  ...
  void enable_tx() {
    auto val = cmac_.read_register(conf_tx);
    val |= 1;
    cmac_.write_register(conf_tx, val);
  }
